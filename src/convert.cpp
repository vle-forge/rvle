...
value::Value* rvle_toVleValue_implicit(SEXP rval, const std::string& rvalClass)
{
    ...
    if(rvalClass == "data.frame"){
        ...
        for(unsigned int col =0; col<length(temp); col++){
            ...
            if(col == 0){
                res = new value::Matrix(length(temp),length(colData)+1,10,10);
            }
...
value::Value* rvle_toVleValue_implicit(SEXP rval)
{
    ...
    case NILSXP:{  //NULL
        ...
        if(length(temp) == length(names)){//value::Map
            ...
            res = new value::Set();
            for(unsigned int i =0; i<length(temp); i++){
                res->toSet().add( reinterpret_cast<value::Value*>(
                        rvle_toVleValue(VECTOR_ELT(temp,i))));
            }
        }
        UNPROTECT(1);//temp
...
SEXP rvle_toRvalue(rvle_value_t vlevalToCast,
        ...
        int matrix_type_depth)
{
    ...
    case value::Value::BOOLEAN: {
        ...
        if(multiple_values && unlist_multiple_values && setVal.size() == 1){
            res = rvle_toRvalue(setVal.get(0),
                    without_class_names,0,0,
            ...
            for (int i = 0; itb!=ite; ++i,itb++) {
                SET_VECTOR_ELT(res,i,rvle_toRvalue(*itb,
                        without_class_names,0,0,
                        matrix_type, matrix_type_depth-1));
            }
            if(without_class_names == 0){
        ...
        //build values
        for (int i = 0; itb!=ite; ++i,itb++) {
            SET_VECTOR_ELT(res,i,rvle_toRvalue(itb->second,
                    without_class_names,0,0,
                    matrix_type, matrix_type_depth-1));
        }
        //build names
        ...
        PROTECT(res = allocMatrix(REALSXP,
                       tableVal.height(),
                       tableVal.width()));
        ...
        SET_STRING_ELT(res, 0,
                mkChar(vleval->toString().value().c_str()));
        if(without_class_names == 0){
        ...
        if(matrix_type == 0 ||  matrix_type_depth > 0){
            ...
            PROTECT(res = allocMatrix(VECSXP,
                    matrixVal.rows(),
                    matrixVal.columns()));
            for(int i =0;i< matrixVal.columns();++i){
                for(int j =0;j< matrixVal.rows();++j){
                    SET_VECTOR_ELT(res, (j+i*matrixVal.rows()),
            ...
            for(unsigned int col = 0; col < matrixVal.columns() ; col++){
                ...
                if(matrixVal.rows() > 1){
                    const value::ConstVectorView& column = matrixVal.column(col);
                    const value::ConstVectorView& dataVec =
                            column[boost::indices[value::Matrix::Range(1,column.size())]];
                    colType = rvle_get_vector_type(dataVec);
                    switch (colType) {
                    case value::Value::BOOLEAN: {
                        PROTECT(value = NEW_LOGICAL(column.size()-1));
                        int i = 0;
                        for (value::ConstVectorView::const_iterator it = dataVec.begin();
                                it != dataVec.end(); ++it) {
                        ...
                        for (value::ConstVectorView::const_iterator it = dataVec.begin();
                            ...
                            if (*it == 0 || (*it)->getType() != value::Value::STRING) {
                                ...
                                SET_STRING_ELT(value, i, mkChar(value::toString(*it).c_str()));
                            }
                            ++i;
                        }
                        UNPROTECT(1);
            ...
            PROTECT(res = allocMatrix(REALSXP,
                    matrixVal.value().shape()[1],
                    matrixVal.value().shape()[0]));
            for (i = 0; i < view.shape()[0]; ++i) {
                for (j = 0; j < view.shape()[1]; ++j) {
                    if (view[i][j]) {
                        switch (view[i][j]->getType()) {
                        case value::Value::BOOLEAN:
                            REAL(res)[j + i * view.shape()[1]] = (double)
...
rvle_value_t rvle_toVleValue(SEXP rval)
{
    ...
    if(rvalClassSexp == R_NilValue){
        ...
        if(rvalClass == "VleBOOLEAN"){
            ...
            for(unsigned int i =0; i<ncol; i++){
                for(unsigned int j =0; j<nrow; j++){
                    res->toMatrix().set(i, j, reinterpret_cast<value::Value*>(
